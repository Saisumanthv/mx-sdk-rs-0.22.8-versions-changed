...
/// Replaces every generic with `<dharitri_wasm::api::uncallable::UncallableApi>` in a type, indiscriminately.
            ...
            if let syn::PathArguments::AngleBracketed(args) = &mut segment.arguments {
                for arg in args.args.iter_mut() {
                    if let syn::GenericArgument::Type(gen_ty) = arg {
                        *gen_ty =
                            syn::parse_str("dharitri_wasm::api::uncallable::UncallableApi").unwrap();
                    }
                }
            }
        }
...
fn type_payload_size(type_name: &syn::Type) -> proc_macro2::TokenStream {
    ...
    quote! {
        <#type_name_replaced as dharitri_wasm::types::ManagedVecItem<dharitri_wasm::api::uncallable::UncallableApi>>::PAYLOAD_SIZE
    }
}
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                quote! {
                    <#type_name as dharitri_wasm::types::ManagedVecItem<dharitri_wasm::api::uncallable::UncallableApi>>::SKIPS_RESERIALIZATION
                }
            })
            .collect(),
        _ => {
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                quote! {
                    #field_ident: dharitri_wasm::types::ManagedVecItem::<M>::from_byte_reader(api.clone(), |bytes| {
                        let next_index = index + <#type_name as dharitri_wasm::types::ManagedVecItem<M>>::PAYLOAD_SIZE;
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                let type_name = &field.ty;
                quote! {
                    dharitri_wasm::types::ManagedVecItem::<M>::to_byte_writer(&self.#field_ident, |bytes| {
                        let next_index = index + <#type_name as dharitri_wasm::types::ManagedVecItem<M>>::PAYLOAD_SIZE;
...
fn generate_array_init_snippet(ast: &syn::DeriveInput) -> proc_macro2::TokenStream {
    ...
    quote! {
        const SELF_PAYLOAD_SIZE: usize = <#self_expr as dharitri_wasm::types::ManagedVecItem<dharitri_wasm::api::uncallable::UncallableApi>>::PAYLOAD_SIZE;
...
pub fn managed_vec_item_derive(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let impl_generics = if ast.generics.params.is_empty() {
        quote! { <M: dharitri_wasm::api::ManagedTypeApi> }
    } else {
    ...
    let gen = quote! {
        impl #impl_generics dharitri_wasm::types::ManagedVecItem<M> for #name #ty_generics #where_clause {
