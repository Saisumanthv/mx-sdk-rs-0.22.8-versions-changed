    ...
    SA: SendApi + 'static,
{
    ...
    fn convert_to_single_transfer_dct_call(mut self) -> Self {
        if let Some(payment) = self.payments.get(0) {
            if payment.token_identifier.is_moax() {
                ...
                let endpoint_name =
                    ManagedBuffer::new_from_bytes(self.api.clone(), DCT_TRANSFER_FUNC_NAME);
                ...
                let zero = BigUint::zero(self.api.clone());
                let endpoint_name =
                    ManagedBuffer::new_from_bytes(self.api.clone(), DCT_NFT_TRANSFER_FUNC_NAME);
    ...
    fn convert_to_multi_transfer_dct_call(self) -> Self {
        ...
        let zero = BigUint::zero(self.api.clone());
        let endpoint_name =
            ManagedBuffer::new_from_bytes(self.api.clone(), DCT_MULTI_TRANSFER_FUNC_NAME);
